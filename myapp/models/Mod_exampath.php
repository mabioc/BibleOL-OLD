...
// TODO: Clean-up unescessary functions

class Mod_exampath extends CI_Model {
    private $exampath;
    private $qpl1; // Length of $exampath + 1
    ...
    public function init(string $path, bool $must_be_dir, bool $check_access, $must_exist = true) {
        ...
        $this->exampath = realpath('exam');
        if (!$this->exampath)
            throw new DataException(sprintf($this->lang->line('cannot_open_folder'), 'exam'));

        $this->qpl1 = strlen($this->exampath)+1;
        ...
        // Verify that we are below the exam directory
        if (strpos($this->canonical_absolute, $this->exampath)!==0)
    ...
    public function is_top() {
        return $this->canonical_absolute === $this->exampath;
    }
    ...
    private function rel2abs(string $path, bool $must_exist) {
        ...
        $real_path = realpath(dirname("exam/$path"));
        if ($real_path)
    ...
    public function fix_exerciseowner(&$added, &$deleted) {
        $exampath = realpath('exam');
        $this->load->helper('directory');
        $d = directory_map("$exampath");
        ...
        foreach ($query->result() as $row) {
            if (!file_exists('exam/' . $row->pathname)) {
